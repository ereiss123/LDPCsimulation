//==============================================================
// redecodeStatistics.cpp
// 
// This program obtains statistics for re-decoding NGDBF
// frames. It produces statistics for repeated decoding
// of frames beginnin from the same initial condition.
// Each received frame is re-decoded NR times in order to
// obtain the frame-specific error probability, Pe(f).
// The final output is a record of all frame results, printed
// to a text file in tab delimited format. Each row represents
// one received frame, and each column entry is the error weight
// at the outcome of one redecode phase. 
//==============================================================
...
int main(int argc, char * argv[])
{
  ...
  while (totalWords < NF)
      ...
      for (i=0; i<H.N; i++)
	{
      ...
      while (phase<NR) {	
	// Reset initial state:
	// Note that the noise perturbation state is not reset
	for (int i=0; i<H.N; i++)
	  {
	    d[i] = r[i];
#ifdef outputSmoothing
	...
	phase++;
      ...
      // ------------------------------------------------
      // Give a status message every 1000 frames
      int reportInterval = 1000;
      if ((totalWords % reportInterval) == 0)
	{
	  cout << "\nIncremental result: " << errors << " bit errs in " << totalWords << " words, BER=" << (double)errors/totalBits 
	       << ". Average iterations = " << (double) totalIterations/(totalWords*NR) << ". Word error=" << wordErrors << ". Uncoded errors = " << uncodedErrors << ", uncBER=" << (double)uncodedErrors/totalBits
	       << "\nError weights:\n";
  ...
  cout << "\nFinal result: " << errors << " bit errs in " 
       << totalWords << " words, BER=" << (double)errors/totalBits << ". Average iterations = " << (double) totalIterations/(totalWords*NR) 
