...
int main(int argc, char * argv[])
{
  ...
  // Get code parameters:
  int dv = H.biggest_num_n; // Highest degree symbol nod
  int dc = H.biggest_num_m; // Highest degree check node
...
void symNodeUpdates(alist_struct &H, vector<double> & y, vector<int> & d, vector<vector<double> > & sym_to_check, vector<vector<double> > & check_to_sym)
{ 
  int cnode, midx;//cnode is the id of the check node, midx is the "port"
  double msg, outmsg, sum;

  for (int i=0; i<H.N; i++)
    {
      sum = y[i]; //initialize symnode to channel msg
      ...
      for (int j=0; j<H.num_nlist[i]; j++) 
	{
	  ...
	  midx = find(H.mlist[cnode],H.num_mlist[cnode],i); 
	  msg = check_to_sym[cnode][midx]; 
	  outmsg = sum - msg; //subtract node being updated from sum
